    ...
    for mat_metadata in mat_info:
        if mat_metadata:
            ...
            get_new_roots_workflow = chain(
                group([
                    get_supervoxel_ids_from_expired_root_ids.s(root, mat_metadata)
                    for root in expired_root_ids],
                    chunk_process_root_ids.s(mat_metadata)))
...
@celery.task(name="process:chunk_process_root_ids",
             ...
             max_retries=3)
def chunk_process_root_ids(self, mat_metadata: dict, supervoxel_data: dict):
    ...
    for __, data in supervoxel_data.items():
        chunked_supervoxels = create_chunks(data, len(data) // 100)
        # get_roots_tasks = [get_new_roots.s(chunk, mat_metadata) for chunk in chunked_supervoxels]
        worker_chain = [chain(get_new_roots.s(chunk, mat_metadata), update_segmentation_table.s(
            mat_metadata)) for chunk in chunked_supervoxels]
...
def get_materialization_info(aligned_volume: str,
    ...
    for annotation_table in annotation_tables:
            segmentation_table_name = f"{annotation_table}__{pcg_table_name}"
...
def get_expire_root_ids(mat_metadata, expired_chunk_size: int=100):
    ...
    expired_chunked_root_id_lists = [x.tolist() for x in [*chunked_root_ids]]
    return expired_chunked_root_id_lists

@celery.task(name="process:get_supervoxel_ids_from_expired_root_ids",
             acks_late=True,
             bind=True,
             autoretry_for=(Exception,),
             max_retries=3)
def get_supervoxel_ids_from_expired_root_ids(self, root_id_chunk: list, mat_metadata: dict):
    """[summary]
    ...
    Returns:
        [type]: [description]
    """
    aligned_volume = mat_metadata.get("pcg_table_name")
    ...
    try:
        for root_id_column in root_id_columns:
            prefix = root_id_column.split("_")
            supervoxel_name = f"{prefix[0]}_{prefix[1]}_supervoxel_id"

            expired_root_id_data[root_id_column] = [
                data
                for data in session.query(SegmentationModel.id,
